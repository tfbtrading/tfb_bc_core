codeunit 50181 "TFB Sales Shipment Mgmt"
{
    ...
    begin
        ...
        //Check that content has been generated to send
        If GenerateShipmentNotificationContent2(RefNo, HTMLBuilder) then begin
                                ...
                                If LastLotNo <> LedgerEntry."Lot No." then begin
                                    ...
                                    If LotInfo.FindFirst() then begin
                                        ...
                                        If (Ref > 0) and (PersBlobCU.Exists(Ref)) then begin
                                            ...
                                            EmailMessage.AddAttachment(FileNameBuilder.ToText(), 'Application/pdf', InStream2);
                                        end;
                                        ...
                                        repeat
                                            If DocAttachment."Document Reference ID".HasValue then begin
                                                ...
                                                if DocAttachment."Document Reference ID".ExportStream(outStreamReportAtc[i]) then begin
                                                    //Mail Attachments
                                                    EmailMessage.AddAttachment(FileNameBuilder.ToText(), 'Application/pdf', inStreamReportAtc[i]);
                                                end;
                                                i += 1;
                                            end;
    ...
    /// <summary> 
    /// Generate html content for transaction emails programatically.
    /// //TODO to be switched at a future point to use declarative template technology
    /// </summary>
    /// <param name="RefNo">Parameter of type Code[20] for the shipment header reference number.</param>
    /// <returns>Return variable "Text".</returns>
    local procedure GenerateShipmentNotificationContent(RefNo: Code[20]): Text
    ...
    begin
        ...
        If Line.FindSet(false) then begin
            ...
            BodyBuilder.AppendLine(StrSubstNo('<h2>Goods have shipped on %1 against our order %2.</h2>', Header."Posting Date", Line."Order No."));
            If Header."External Document No." <> '' then
                BodyBuilder.AppendLine(StrSubstNo('<h3>Your order reference is %1 </h3>', Header."External Document No."));

            If Customer."TFB CoA Required" then
                BodyBuilder.AppendLine(StrSubstNo('<p>%1 has requested that Certificates of Analysis be sent</h3>', Customer.Name));

            if Header."TFB 3PL Booking No." <> '' then
                BodyBuilder.AppendLine(StrSubstNo('<p> Document received may contain a reference to our 3PL booking no %1 </p>', Header."TFB 3PL Booking No."));
            ...
            BodyBuilder.Append('<th class="tfbdata" style="text-align:left" width="15%">Qty Ordered</th>');
            BodyBuilder.Append('<th class="tfbdata" style="text-align:left" width="15%">Weight</th>');
            BodyBuilder.Append('<th class="tfbdata" style="text-align:left" width="30%">Comments</th></thead>');
                ...
                If Item.Type = Item.Type::Inventory then begin
                    ...
                    If Line."Drop Shipment" then begin
                        ...
                        If ExpectedDate = Header."Posting Date" then
                            CommentBuilder.AppendLine('Delivered today')
                        else
            ...
            OrderLine.SetFilter("Outstanding Qty. (Base)", '>%1', 0);
            OrderLine.SetRange("Document Type", OrderLine."Document Type"::Order);

            if OrderLine.FindSet(false, false) then begin

                BodyBuilder.AppendLine(StrSubstNo('<H3> The following items still have to be shipped against %1</H3>', OrderLine."Document No."));
    ...
    /// <returns>Return variable "Boolean".</returns>
    local procedure GenerateShipmentNotificationContent2(RefNo: Code[20]; var HTMLBuilder: TextBuilder): Boolean
